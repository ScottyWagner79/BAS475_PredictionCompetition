...
credit_TR %>% 
  ...
  model(
    ARIMA = ARIMA(box_cox(Credit, lambda)),
    NAIVE = NAIVE(box_cox(Credit, lambda)),
    DAMP_Holt = ETS(box_cox(Credit, lambda) ~ error("A") + trend("Ad", phi = .9) + season("N"))
  ) %>% 
  forecast(h = 12) %>% 
  accuracy(CREDIT)
...
#autoplot of model on original data
fit %>% 
  forecast(h = 12) %>% 
  autoplot(CREDIT) +
  labs(y = "Credit", title = "Credit Earned Over Time")
...
pred2 <- pred[,4] #storing credit predictions 
pred2 
write.csv(pred2,"predictions.csv") #
```
